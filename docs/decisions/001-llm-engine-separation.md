...
Determinism is a requirement. The current architecture guarantees that `executeScenario(operation)` returns identical output for identical input regardless of whether the LLM provider is GitHub Models (currently `openai/gpt-4.1`) or Ollama running locally. Switching providers, updating model versions, or changing LLM temperature settings cannot affect financial results. This makes the tool defensible in a PM context: if a manager asks "why does the margin come out to 32.5%?", the answer is a reproducible chain of arithmetic in `server/engine/margin.ts`, not a stochastic model whose behavior may have changed since the analysis was run.

Testability was the third factor. The engine in `server/engine/` has no dependency on `server/ai.ts`, no external I/O, and no API credentials. As of this writing, most calculation modules carry a dedicated unit-test file — `budget.test.ts`, `evm.test.ts`, `labor.test.ts`, `margin.test.ts`, `narrative.test.ts`, `scenarios.test.ts`, and `utilization.test.ts` — plus `validation.test.ts` for the `scenarioOperationSchema` Zod contract that guards the boundary described above. `portfolio.ts` (portfolio aggregation) and `matching.ts` (fuzzy project/role resolution) have no dedicated file of their own; they're exercised transitively through `executor.ts`'s test suite (`executor-guards.test.ts`, `deterministic-asofdate.test.ts`, `evm-proxy.test.ts`), which calls `executeScenario()` the same way production does — see the README's "Coverage scope" note for why the Playwright E2E suite does not additionally cover this path. `goal-seeking.test.ts` is not a per-module file — there is no `goal-seeking.ts` — it checks that `labor`/`margin`/`budget`/`scenarios` combine correctly the way the V3 agentic loop chains them to answer goal-seeking-style queries. Every calculation path — including EVM metrics like CPI, SPI, EAC, and TCPI, and composite what-if scenarios — can be verified in CI without a network connection or API key. This test suite would be impossible to write if the LLM were doing the arithmetic, because the LLM's output cannot be reliably predicted or asserted.
...
- The engine must cover every operation type explicitly. Adding a new scenario type requires implementing it in the engine, not just updating a prompt.
- Intent parsing quality affects which operation is invoked. A misclassified query routes to the wrong calculation function; the numbers will be accurate for the wrong scenario. Two distinct cases exist here. An ambiguous-but-parseable query makes the model itself default to `burn_rate_check`, per an explicit rule in `PARSE_INTENT_PROMPT` (`server/ai.ts`) — that is a successful parse of a generic operation, not an error path. A genuinely unparseable response (malformed JSON, a schema-invalid operation, a provider/transport failure, or an unconfigured provider) is a typed `IntentParseFailure` (`server/ai.ts`, `IntentParseFailureCode`) that the API layer (`sendIntentParseFailure()`, `server/routes.ts`) surfaces as HTTP 422 — it does not silently substitute `burn_rate_check`.
